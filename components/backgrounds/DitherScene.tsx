...
const RetroEffect = forwardRef<RetroEffectImpl, { colorNum: number; pixelSize: number }>(function RetroEffect(props, ref) {
...
function DitheredWaves({
  ...
  useEffect(() => {
    ...
    const resolution = uniforms.resolution.value as THREE.Vector2;
  ...
  useFrame(({ clock }) => {
    // React Three Fiber expects imperative uniform updates on each frame.
    const uniforms = materialRef.current?.uniforms;
    if (!uniforms) return;
...
export default function DitherScene({
  ...
  return (
    <Canvas className="dither-canvas" camera={{ position: [0, 0, 6] }} dpr={[1, 1.25]} gl={{ antialias: false, powerPreference: 'low-power' }}>
      <DitheredWaves
        waveSpeed={0.05}
        waveFrequency={3}
        waveAmplitude={0.38}
        waveColor={[0.33, 0.45, 1]}
        colorNum={4}
        pixelSize={pixelSize}
